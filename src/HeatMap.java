...
public class HeatMap extends JPanel
{
     ...
     */
    public HeatMap(double[][] data, boolean useGraphicsYAxis, Color[] colors, ArrayList<String> go)
    {
        super();
    ...
    public void paintComponent(Graphics g)
    {
        ...
        int numXTicks = goIDS.size();
        //int numYTicks = (height - 60) / 50;
        int numYTicks = goIDS.size();
            ...
            for (int x = 1; x <= numXTicks; x++)
            {
             ...
             // clockwise 90 degrees
               ...
               // g2d.setTransform(at);
                ...
                /*int labelX = (31 + x * xDist) - 4 * label.length();           
                //int v=g.getFontMetrics(getFont()).getHeight()+1;
                int v=g.getFontMetrics(getFont()).getHeight()-5;
                int j =0;
                int k= label.length();
                while(j < k+1)
                {
                  if (j == k) 
                     g2d.drawString(label.substring(j),30 + x * xDist, (height-90)+(j*v));
                  else
                     g2d.drawString(label.substring(j,j+1),30 + x * xDist, (height-90)+(j*v));
                  j++;
                 }*/
            }
        }
        ...
        if (drawLegend)
        {
        	...
        	g2d.drawRect(width - 29, 30, 10, height - 60);
            for (int y = 0; y < height - 61; y++)
            {
               	...
               	if(y==0)
            	{
               		...
               		g2d.setFont(font);
            		String legend = ""+smallest;
            		String title = legend.substring(0, 3);
            		g2d.drawString(title, width - 16, yStart);
            	}
               	if(y==(height-61-1))
            	{
               		Font font = new Font("Arial", Font.PLAIN, 9);
