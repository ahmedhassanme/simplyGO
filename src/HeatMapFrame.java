...
class HeatMapFrame extends JFrame
{
    ...
    public HeatMapFrame(double[][] info, ArrayList<String> go) throws Exception
    {
        ...
        panel = new HeatMap(data, useGraphicsYAxis, Gradient.GRADIENT_BLUE_TO_RED, go);
        ...
        panel.setTitle("GO Semantic Similarity (Information Content)");
        panel.setDrawTitle(true);

        panel.setXAxisTitle("Genes");
        panel.setDrawXAxisTitle(true);

        panel.setYAxisTitle("Genes");
        panel.setDrawYAxisTitle(true);

        panel.setCoordinateBounds(0, Double.parseDouble(""+data.length), 0, Double.parseDouble(""+data.length));
